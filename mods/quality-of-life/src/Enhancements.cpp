...
Enhancements::Enhancements(const Core::Logger& logger, EnhancementsFeatures& enhancementsFeatures)
    :
    m_Logger(logger),
    m_EnhancementsFeatures(enhancementsFeatures),
    m_PatchAnyVehicleTypeOnline
    {
...
{
    if (m_EnhancementsFeatures.AnyVehicleTypeOnline)
    {
        // Apply any vehicle type online patch.
        {
            m_Logger.Info("Applying any vehicle type online patch...");

            m_PatchAnyVehicleTypeOnline[0].Apply("\xEB\x12");
            m_PatchAnyVehicleTypeOnline[1].Apply("\x90\x90\x90\x90\x90");

            m_Logger.Info("Applied any vehicle type online patch.");
        }
    }

    if (m_EnhancementsFeatures.DisableInfiniteGears)
    {
...
void Enhancements::Unload()
{
    ...
    if (m_EnhancementsFeatures.AnyVehicleTypeOnline)
    {
        // Remove any vehicle type online patch.
        {
            ...
            m_PatchAnyVehicleTypeOnline[0].Remove();
            m_PatchAnyVehicleTypeOnline[1].Remove();
